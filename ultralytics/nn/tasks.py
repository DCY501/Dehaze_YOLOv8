...
class BaseModel(nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        csd = intersect_dicts(csd, target_state)  # intersect

        # Dehaze models insert DehazeHead before Detect, so pretrained YOLO Detect keys
        # such as model.22.* need to be copied to the shifted Detect layer, e.g. model.23.*.
        source_layers = getattr(model, 'model', None)
        target_layers = getattr(self, 'model', None)
        if isinstance(source_layers, nn.Sequential) and isinstance(target_layers, nn.Sequential):
            source_detect = [i for i, m in enumerate(source_layers) if isinstance(m, (Detect, Segment))]
            target_detect = [i for i, m in enumerate(target_layers) if isinstance(m, (Detect, Segment))]
            if source_detect and target_detect and source_detect[-1] != target_detect[-1]:
                src_prefix = f'model.{source_detect[-1]}.'
                dst_prefix = f'model.{target_detect[-1]}.'
                for k, v in model.float().state_dict().items():
                    if k.startswith(src_prefix):
                        mapped_key = dst_prefix + k[len(src_prefix):]
                        if mapped_key in target_state and mapped_key not in csd and v.shape == target_state[mapped_key].shape:
                            csd[mapped_key] = v
